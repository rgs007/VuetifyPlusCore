    ...
    <v-slide-y-transition mode="out-in">
      <v-row>
        <v-col>
          ...
          <p>This component demonstrates checkauth data from the server.{{msg}}</p>
...
export default Vue.extend({
  ...
  methods: {
    getColor(temperature: number) {
      ...
      async authorize() {
          try {
              const response = await this.$axios.get<string>('https://svaptsimad160.fomento.es/autologin/api/login', { withCredentials: true });

              this.msg = response.data;
          }
          catch (e) {
              this.showError = true;
              this.errorMessage = `Error while loading weather authorize: ${e.message}.`;
          }
          this.loading = false;
      }
      ,
